...
1. Polar Modulated Transmitter (Original Approach)
...
2. QSE-Based Modulator (Conventional Approach)
...
An alternative implementation uses an external Si5351 clock generator. In this configuration, achieving phase modulation requires a different strategy: instead of directly controlling a digital NCO, the system must apply rapid frequency updates to the Si5351 over SPI in order to indirectly modulate phase. This approach is conceptually similar to the technique used in the uSDX transceiver architecture, where fast frequency stepping is used to approximate continuous phase modulation.

Reference to uSDX-style frequency update approach:
[Insert link to uSDX project / Si5351 fast update method]

While this method introduces additional latency and constraints compared to an internal NCO, it may offer improved isolation from digital noise sources and simplify RF generation in some configurations.
...
[Insert schematic of polar modulation hardware test-bed here]
...
The key idea is to select the system clock frequency that most closely aligns with an integer multiple of the desired NCO frequency. By doing so, the number of fractional clock corrections required in the waveform generation process is reduced. This minimises the occurrence of “one-clock adjustments” in the output stream, which are a major contributor to spurious spectral energy.

[Insert graph showing adaptive clock selection improving frequency accuracy and reducing spurs here]
...
Initially, results from this configuration were somewhat disappointing, with higher-than-expected noise and reduced clarity in the transmitted signal. However, it was found that improving the quality of the audio input path had a significant impact on overall performance. In particular, increasing ADC resolution through oversampling reduced quantisation noise and improved the stability of the modulation signal fed into the frequency control loop.
...
Compare output from all 4 branches
